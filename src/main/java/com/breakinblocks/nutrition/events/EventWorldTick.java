...
import com.breakinblocks.nutrition.effects.EffectsManager;
...
public class EventWorldTick {
    ...
    private Map<Pair<EntityPlayer, Boolean>, Integer> playerFoodLevels = new HashMap<>(); // Track food level between
                                                                                          // ticks
    ...
    private void potionTicking(World world) {
        if (potionCounter > 110) {
            for (EntityPlayer player : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList()
                    .getPlayers()) // All players on server
