...
public final class DietTierEvaluator {
    ...
    public static void evaluate(ServerPlayer player) {
        ...
        Optional<ResourceLocation> activeId = active != null
                ? Optional.of(active.key().location())
                : Optional.empty();

        Map<ResourceLocation, PlayerModifierTracker.ModifierSpec> tierModifiers = new HashMap<>();
        if (active != null) {
            ResourceLocation tierId = active.key().location();
            for (AttributeModifierEntry mod : active.value().attributeModifiers()) {
                ResourceLocation modId = Nutritional.id("tier/" + tierId.getNamespace() + "/" + tierId.getPath() + "/" + mod.attribute().getRegisteredName().replace(':', '/'));
    ...
    public static boolean matches(TierCondition condition, PlayerNutritionData data, Registry<NutrientDefinition> nutrients) {
        ...
        for (Map.Entry<ResourceLocation, RangeSpec> entry : condition.perNutrient().entrySet()) {
            sawAny = true;
            NutrientDefinition def = nutrients.get(entry.getKey());
            if (def == null) {
    ...
    private static float averageValue(PlayerNutritionData data, Registry<NutrientDefinition> nutrients) {
        ...
        for (Holder.Reference<NutrientDefinition> ref : nutrients.holders().toList()) {
            total += data.get(ref.key().location(), ref.value().defaultValue());
            count++;
        }
    ...
    private static float minValue(PlayerNutritionData data, Registry<NutrientDefinition> nutrients) {
        ...
        for (Holder.Reference<NutrientDefinition> ref : nutrients.holders().toList()) {
            float v = data.get(ref.key().location(), ref.value().defaultValue());
            if (v < min) min = v;
        }
