package com.breakinblocks.nutrition.gui;
...
public class NutritionGui extends GuiScreenDynamic {
        ...
        for (Nutrient nutrient : NutrientList.getVisible()) {
            ...
            float currentNutrient = (ClientProxy.localNutrition != null &&
                    ClientProxy.localNutrition.get(nutrient) != null) ?
                            Math.round(ClientProxy.localNutrition.get(nutrient)) : 0; // Display empty if null
            int nutritionBarDisplayWidth = (int) (currentNutrient / 100 * NUTRITION_BAR_WIDTH);

            // Draw icons
            itemRender.renderItemIntoGUI(nutrient.icon, left + NUTRITION_ICON_HORIZONTAL_OFFSET,
    ...
    public void initGui() {
        ...
        for (Nutrient nutrient : NutrientList.getVisible()) {
            int nutrientWidth = fontRenderer
                    .getStringWidth(I18n.format("nutrient." + Tags.MODID + ":" + nutrient.name)); // Get width of
                                                                                                  // localized string
        ...
        super.updateContainerSize(GUI_BASE_WIDTH + labelCharacterPadding,
                GUI_BASE_HEIGHT + (NutrientList.getVisible().size() * NUTRITION_DISTANCE));
    ...
    public void redrawLabels() {
        ...
        labelList.add(
                label = new GuiLabel(fontRenderer, 0, (width / 2) - (fontRenderer.getStringWidth(nutritionTitle) / 2),
                        top + TITLE_VERTICAL_OFFSET, 0, 0, 0xffffffff));
        ...
        for (Nutrient nutrient : NutrientList.getVisible()) {
            ...
            labelList.add(label = new GuiLabel(fontRenderer, 0, left + LABEL_NAME_HORIZONTAL_OFFSET,
                    top + LABEL_VERTICAL_OFFSET + (i * NUTRITION_DISTANCE), 0, 0, 0xffffffff));
            ...
            labelList.add(
                    label = new GuiLabel(fontRenderer, 0, left + LABEL_VALUE_HORIZONTAL_OFFSET + labelCharacterPadding,
                            top + LABEL_VERTICAL_OFFSET + (i * NUTRITION_DISTANCE), 0, 0, 0xffffffff));
