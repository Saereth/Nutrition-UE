...
public class EventPlayerDeath {
    ...
    public void onPlayerClone(PlayerEvent.Clone event) {
        ...
        INutrientManager nutritionOld = event.getOriginal().getCapability(NUTRITION_CAPABILITY, null); // Get old
                                                                                                       // nutrition
        ...
        if (event.isWasDeath()) {
            ...
            for (Nutrient nutrient : nutrition.keySet())
                if (Config.deathPenaltyReset || nutrition.get(nutrient) > Config.deathPenaltyMin) // If reset is
                                                                                                  // disabled, only
                                                                                                  // reduce to cap when
                                                                                                  // above its value
                    player.getCapability(NUTRITION_CAPABILITY, null).set(nutrient,
                            Math.max(Config.deathPenaltyMin, nutrition.get(nutrient) - Config.deathPenaltyLoss)); // Subtract
                                                                                                                  // death
                                                                                                                  // penalty
                                                                                                                  // from
                                                                                                                  // each
